...
def get_explanation_from_position(start, end, text=None, sentences=None):
    ...
    for sentence_id, sentence in enumerate(sentences):
        if ((start >= count) and (start<count+len(sentence))) and ((end > count) and (end<=count+len(sentence))):
            return sentence, sentence_id
        count+=(len(sentence)+1)
    return None, None

def get_substext_starts(subtext, text):
    matches = re.finditer(subtext, text)
